...
marcin_dead = fread(here("data/scenario_SAX-cqb-deaths-cumulative-Total_False_2022_03_21_year_end_EN.csv"))
...
marcin_testpos = fread(here("data/scenario_SAX-detections-Total_False_2022_03_21_year_end_EN.csv"))
...
marcin_hosp = fread(here("data/scenario_SAX-hospitalized-Total_False_2022_03_21_year_end_EN.csv"))
...
p_marcin_dead= ggplot(marcin_dead[DateRep<=as_date(plotenddateTOT)& DateRep>=as_date("2022-03-01")], aes(DateRep, mean, col = scenario2, fill = scenario2)) +
  ...
  geom_line(data= observed_data2[DateRep >=as_date(redaktionsschlus_datum),. (DateRep, mean = AllDeaths , scenario2 = "Berichtet")] , lwd = 1, alpha = 0.9, col = colornewdata) + 
  guides(color = "none")+
...
jpeg(here("results/b22git_s03_1_comparison_SimuBulletin22_vs_observed.jpeg"),width = 15,6, res = 150, quality= 100, units = "in")
p_posit +guides(fill = 'none', col = "none") + p_marcin_hosp + p_marcin_dead + plot_layout(nrow = 1,guides = 'collect') + plot_annotation(caption =  paste0("Datenstand Inzidenz:", datenstand, "\n", "Datenstand Hospitalisierte:", datenstandnormalstation) %>% as.character)& theme(legend.position = 'top') 
